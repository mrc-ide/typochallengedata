...
for(i in 1:n_erroneous)
{
  ...
  if(recov_date_correct[i]) error_class[i] <- "recovered_correct"
  if(!recov_is_date[i]) error_class[i] <- "no_date_recovered"
  
  if(is_internal_swap(erroneous_ret$date[i],erroneous_ret$user[i])){
    error_class[i] <- "internal_swap" }
  
  if(is_external_swap(erroneous_ret$date[i],erroneous_ret$user[i])){
  error_class[i] <- "external_swap" }
  
  mutation_flag <- is_mutation(erroneous_ret$date[i],erroneous_ret$user[i])
  if(mutation_flag != 0){
  if(mutation_flag==1) error_class[i] <- "neighbour_swap" 
  if(mutation_flag==2) error_class[i] <- "random_swap"
  }
